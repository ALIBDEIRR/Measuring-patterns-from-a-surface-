...
//string pcd_name = "half_sphere_80.pcd";	// half sphere pcd in local ROI
...
string pcd_name = "ball1000.pcd";

//-----------------------------------------------------------------Define variables---------------------------------------------------------------------------------------------------------
#define threshold 0.01     // minimum distace for a point in the dataset to be considered as an inliers.
#define INTERSECTION_THRESHOLD 0.001  // Distance between the fitted sphere and the points in the fitted point cloud. 
int t{0};
char confirmation;
...
int main() {
    ...
    cout << "--------------------------------------------------------------------------------------"<<endl;
    cout << "Do you want to add noise to the cloud ? (Y/N) " << endl;
    cin >> confirmation;
...
//------------------------------------------------------------- computeModelCoefficients()   -   PCL SAC MODEL SPHERE  --------------------------------------------------------------------------------
    ...
    cout << "Center of the fitted sphere before opt is: " << non_optimized_center.x << " " << non_optimized_center.y << " " << non_optimized_center.z << endl;
    cout << "-------------------------------------------------------------"<<endl;
...
// ------------Here, the model coefficients we obtained form the ransac.computemodel() are used as an input to the optimization fucntion-----------------------------------------------------------------  
    ...
    cout << "Center of the fitted sphere is: " << center.x << " " << center.y << " " << center.z << endl;
    cout << "------------------------------------------------------------------------------- "<<endl;
...
// ------------------------------------------------------------Identify intersection points-----------------------------------------------------------------------------------------
    ...
    for (size_t i = 0; i < cloud->size(); ++i) {
        ...
        if (abs(distance - radius) < INTERSECTION_THRESHOLD) {
            intersection_cloud->points.push_back(point);
        }
    }
    
    cout << "------------------------------------------------------------------------------------- "<<endl;
    cout << "The number of points fitted by the cylinder is : " << intersection_cloud->size() << " data points. " << endl;
    cout << "Threshold value is: "<< INTERSECTION_THRESHOLD << endl;
